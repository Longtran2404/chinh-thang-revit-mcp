...
Script chạy test, build add-in và tạo server độc lập trong `artifacts/ChinhThangRevitMcp-1.2.1-win-x64/`, kèm ZIP và SHA-256. Máy sử dụng bộ ZIP không cần .NET SDK. Không phân phối Revit API DLL, mô hình hay khóa truy cập.
...
Lệnh đầu kiểm tra handshake và 246 tools. Lệnh `--live` phải trả về view thực trong Revit mới chứng minh kết nối đầu cuối hoạt động. Việc build/test thành công không thay thế bước này.
