...
## Checked anchors and explicit stiffeners (2026-09-19)

`revit_create_checked_concrete_anchors(request_json)` places an editable loaded family and validates the actual straight cylindrical shank diameter, axial length, and complete solid containment in a concrete rebar host. Inputs: `host_id`, `symbol_id`, `diameter_mm`, `length_mm`, `diameter_parameter`, `length_parameter`, `positions_mm:[[x,y,z]]`, optional `axis:[0,0,1]` (family local Z), `dry_run:true` by default. Dimensions are mm. Diameter and length appear in Mark for tags/schedules. No line-only or DirectShape substitute is generated. Unhosted one-level or work-plane-based families are supported; work-plane placement creates reference planes in the same reversible transaction.

The tool rejects actual solid protrusion through sides, bottom, top or openings, wrong geometric diameter/length, and unsupported exposed-thread requests. A failed anchor rolls back the entire batch; dry-run commits to validate Revit failures then rolls back. It does not change shared type dimensions. The selected type must already match, or have writable instance dimensions. This bounded tool covers the fully embedded straight shank component; a real base connection's exposed head/thread, bend/head anchorage, cover, edge distance, product qualification and capacity need a separate reviewed detail. Concrete containment does not prove TCVN design compliance.

Smart steel nodes now accept `connection_behavior:Unspecified|Pinned|Moment`. Moment requires `design_detail_reference`; saved type rules use `BeamToColumn_Moment` etc. Rigidity and capacity remain explicitly unverified. Optional `stiffeners` creates actual editable families atomically with the detailed connection. Each entry needs `symbol_id`, `offset_mm` in world axes from the physical node, `axis` for local Z, and `width_mm`, `height_mm`, `thickness_mm` with their corresponding `*_parameter` names. Offsets are bounded to 1000 mm. This is explicit placement, not automatic sizing, weld design, collision clearance or proof of stiffener fit.

Optional `bolt_layout` on a smart-node request checks **TCVN 5575:2024, 14.2.2, Table 43 (note 1), and 14.2.3**. Supported scope is ordinary grade B bearing joints with cut edges, round holes, complete rectangular non-staggered grids and fy <= 540 MPa. Required fields: `standard:TCVN5575:2024`, `joint:Bearing`, `accuracy:B`, `edge:Cut`, `bolt_diameter_mm`, `hole_diameter_mm`, `outer_thinnest_plate_mm`, `fy_mpa`, `width_mm`, `height_mm`, `force_axis:X|Y`, `holes_mm:[[x,y]]`. It checks hole allowance, minimum pitches/edges and conservatively applies outer-row maximum pitch to every adjacent grid row. It rejects failures before creating a node. This numeric input check does not prove that a third-party native connection generated the same holes; response flags that separately. Anchor bolts, friction joints and cold-formed purlins are outside this checker.

Source: [TCVN catalogue](https://tieuchuan.vsqi.gov.vn/tieuchuan/view?sohieu=TCVN+5575%3A2024), standard Table 43 on printed page 106, checked against the [standard PDF](https://icci.vn/storage/documents/December2024/TCVN-5575-2024-tieu-chuan-thiet-ke-ket-cau-thep.pdf). No standard PDF is redistributed in the public repository.

## Verification status

Calculation/layout unit tests and protocol/build results are recorded in [verification.md](verification.md). Native new-feature runtime status must be checked there separately; a successful build is not proof of live Revit geometry.


## Rebar connection guards — 1.2.1

`revit_audit_rebar_connections(request_json)` accepts explicit `rebar_ids` (1–250 sets) and optional `max_gap_mm` (default 500; maximum 2000). It reads every existing bar position using transformed native centrelines. It reports complete-path duplicates, parallel straight-body penetration and coaxial terminal gap candidates between different hosts. Findings include both element IDs and bar-position indices. Select both sides of each joint. Limits: 6000 bars, 4 million eligible bar pairs, 999 findings; an exceeded limit fails explicitly instead of returning a partial pass. It does not infer intentional free ends, coupler validity, required lap length, curved/nonparallel clashes or load paths. `NO_FINDINGS_IN_BOUNDED_CHECK` is never a construction pass.

Creation via `create_rebar_path`, `create_rebar_set`, and `create_rebar_stirrup` now rejects duplicate complete paths and penetrating parallel straight segments against nearby native Rebar, including different hosts and positions within a new set. This also protects designed bars, splice pairs and supports that use the path handler. The operation rolls back; it does not delete existing reinforcement. Tangential/separated parallel lap bars are not classified as body penetration. This is not a full clash/spacing check. Direct C# API scripts, manual Revit edits, baked tools and other mutation tools do not automatically inherit these guards.

**Breaking input requirement for designed rebar:** each enabled `start_anchor`/`end_anchor` must include `receiving_host_id` pointing to the actual concrete receiver. Each disabled anchor must include `disabled_reason`; a reason does not certify the continuation. Old stored recipes require these additional inputs on the next managed replacement. After creation, the tool checks the calculated length from each actual bar end against the receiver's solid/curve intersections, including openings and every distribution position. Overlapping solid intersection intervals are merged. Invalid placement rolls back the whole operation, including restoration of a replaced bar. The checked object is the **centreline**, not a radius-expanded bar or minimum cover envelope. Critical sections remain explicit user inputs; this check cannot validate an incorrectly supplied design station or certify that a slab/column/beam joint is continuous.

Mandatory workflow: inspect connected hosts and source details; use designed anchors or explicit reviewed splice/coupler details for structural end conditions; audit both sides of the joints; inspect cover, crossings and bends; then update schedules/drawings. A bent path must explicitly continue through flight/landing transitions. Do not place independent straight sets up to host faces and label the assembly complete. Do not assume 40d for every project or join bars by making their solids overlap. Failures need a corrected detail, not shorter anchors or removal of checks.

The new checks do not automatically repair previously disconnected reinforcement. Tests are in `scripts/verify-rebar-guards-live.cs`; use a scratch document. Current limitations and installation status are in verification.md.
